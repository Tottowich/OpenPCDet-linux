...
def numpy_to_label_dict(data):
    ...
    if len(data)==0:
        ...
        print(data.shape)
        pred_dict["pred_boxes"] = data[:,:9].astype(np.float32)
        pred_dict["pred_labels"] = data[:,9]
        pred_dict["pred_labels_id"] = data[:,10].astype(np.int32)
        pred_dict["pred_scores"] = data[:,11].astype(np.float16)
    return pred_dict
    ...
    for index in range(0,len(dir)//2):
        ...
        if index==0:
            ...
            vis.update(points=data_dict['points'][:,:-1], 
                        ...
                        pred_labels=pred_dicts['pred_labels_id'],
